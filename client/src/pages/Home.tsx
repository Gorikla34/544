...
const gifts: GiftItem[] = [
  ...
  { id: "easter-egg", name: "Easter Egg", collection: "Telegram Collectible", image: "https://nft.fragment.com/gift/easteregg-1.medium.jpg", stars: 95, ton: "0.27", available: "1 870", icon: "", accent: "loot", rarity: "Common" },
  { id: "artisan-brick", name: "Artisan Brick", collection: "Telegram Collectible", image: "https://nft.fragment.com/gift/artisanbrick-1.medium.jpg", stars: 120, ton: "0.49", available: "588", icon: "", accent: "bear", rarity: "Rare" },
  { id: "genie-lamp", name: "Genie Lamp", collection: "Telegram Collectible", image: "https://nft.fragment.com/gift/genielamp-1.medium.jpg", stars: 130, ton: "0.62", available: "704", icon: "", accent: "potion", rarity: "Rare" },
  { id: "scared-cat", name: "Scared Cat", collection: "Telegram Collectible", image: "https://nft.fragment.com/gift/scaredcat-1.medium.jpg", stars: 105, ton: "0.36", available: "1 132", icon: "", accent: "bunny", rarity: "Rare" },
  { id: "diamond-ring", name: "Diamond Ring", collection: "Telegram Collectible", image: "https://nft.fragment.com/gift/diamondring-1.medium.jpg", stars: 145, ton: "0.84", available: "392", icon: "", accent: "cap", rarity: "Limited" },
  { id: "berry-box", name: "Berry Box", collection: "Telegram Collectible", image: "https://nft.fragment.com/gift/berrybox-1.medium.jpg", stars: 80, ton: "0.16", available: "2 560", icon: "", accent: "heart", rarity: "Common" },
  { id: "snow-mittens", name: "Snow Mittens", collection: "Telegram Collectible", image: "https://nft.fragment.com/gift/snowmittens-1.medium.jpg", stars: 115, ton: "0.44", available: "890", icon: "", accent: "pepe", rarity: "Rare" },
];
...
function giftTonPrice(stars: number) {
  return (stars / STARS_PER_TON).toFixed(2);
}
...
export default function Home() {
  ...
  const payWithStars = async () => {
    try {
      ...
      if (result.invoiceUrl) {
        if (webApp?.openInvoice) webApp.openInvoice(result.invoiceUrl, (status) => {
          if (status === "paid") {
            setBalanceStars((current) => current + starsAmount);
            toast.success(`Зачислено ${starsAmount} ⭐`);
  ...
  const renderHome = () => (
    <>
      ...
      <section className="balance-card">
        ...
        <div className="balance-row"><div><div className="balance-value"><Star size={20} fill="currentColor" /> {balanceStars.toLocaleString("ru-RU")}</div><div className="balance-secondary"><span className="ton-dot">◆</span> {(balanceStars / STARS_PER_TON).toFixed(2)} TON</div></div><button className="ghost-icon-button" onClick={() => changeTab("profile")} aria-label="Профиль"><ChevronRight size={18} /></button></div>
  ...
  const renderProfile = () => (
    <>
      ...
      <section className="profile-balance"><div className="section-label"><span>ДОСТУПНО</span><Star size={14} fill="currentColor" /></div><div className="profile-stars"><Star size={25} fill="currentColor" /> {balanceStars.toLocaleString("ru-RU")}</div><div className="profile-ton">◆ {(balanceStars / STARS_PER_TON).toFixed(2)} TON <span>курс 100 ⭐ = 1 TON</span></div><button className="primary-button" onClick={() => openTopUp("stars")}><Plus size={16} /> Пополнить баланс</button></section>
  ...
  return (
    <main className="app-shell">
      ...
      {selectedGift && <div className="modal-backdrop" onClick={() => setSelectedGift(null)}><div className="gift-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelectedGift(null)}><X size={18} /></button><GiftArtwork gift={selectedGift} large /><div className="modal-content"><div className="modal-kicker"><span className="rarity-chip">{selectedGift.rarity}</span><span><span className="live-dot" /> verified collection</span></div><h2>{selectedGift.name}</h2><p>{selectedGift.collection} · resale lot #{selectedGift.id.slice(0, 4).toUpperCase()}</p><div className="modal-price-row"><div><small>Цена в Telegram Stars</small><strong><StarsMark amount={selectedGift.stars} /></strong></div><div className="modal-ton"><small>По курсу Gift House</small><strong>◆ {giftTonPrice(selectedGift.stars)} TON</strong></div></div><button className="primary-button full-button" onClick={() => { setSelectedGift(null); openTopUp("stars"); }}>Купить за {selectedGift.stars} ⭐ <ArrowUpRight size={17} /></button><div className="modal-footnote"><ShieldCheck size={14} /> NFT передаётся после подтверждения сделки</div></div></div></div>}
      {topUpMode === "stars" && <div className="modal-backdrop" onClick={() => setTopUpMode(null)}><div className="topup-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setTopUpMode(null)}><X size={18} /></button><div className="topup-icon"><Star size={28} fill="currentColor" /></div><span className="eyebrow">TELEGRAM PAYMENTS API</span><h2>Пополнить баланс</h2><p>Счёт откроется прямо в Telegram. Валюта платежа — XTR.</p><div className="rate-banner"><span>Курс пополнения</span><strong>100 ⭐ = 1 TON</strong></div><div className="amount-grid">{[100, 200, 500, 1000].map((amount) => <button key={amount} className={starsAmount === amount ? "amount-chip active" : "amount-chip"} onClick={() => setStarsAmount(amount)}><Star size={13} fill="currentColor" /> {amount}</button>)}</div><label className="amount-label">Сумма Stars<input type="number" min={1} max={100000} value={starsAmount} onChange={(event) => setStarsAmount(Number(event.target.value))} /></label><div className="conversion-preview"><span>Эквивалент по курсу</span><strong>{(starsAmount / STARS_PER_TON).toFixed(2)} TON</strong></div><button className="primary-button full-button" disabled={createInvoice.isPending} onClick={payWithStars}>{createInvoice.isPending ? "Создаём счёт…" : `Оплатить ${starsAmount} ⭐`} <ArrowUpRight size={17} /></button><div className="modal-footnote"><ShieldCheck size={14} /> После подтверждения Telegram Stars добавятся в текущий баланс</div></div></div>}
    </main>
  );
}
